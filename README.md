...
- **97 automated tests**, run in a clean virtualenv against only `requirements.txt` (see "What's verified" below) — not "it looked right in the notebook output."
...
`retriever_name` on `/query` and `/agent/query` accepts: `similarity` (default) | `mmr` | `similarity_score_threshold` | `multi_query` | `parent_document` | `self_query`.
...
97 tests, ~3 seconds, no network access required.
...
- All 5 retrievers, including MMR actually diversifying a near-duplicate result set (not just running without error) and self-query actually applying an LLM-parsed metadata filter
...
- The full FastAPI layer via `TestClient`, including file upload and error responses
...
- No re-ranking stage (e.g. a cross-encoder pass after initial retrieval) — would sit naturally between `VectorStoreRetriever` and the final top-k cut.
...
├── retrievers/              # similarity/mmr/threshold, multi_query, parent_document, self_query
├── agent/                  # tools.py (RetrieverTool) + orchestrator.py (AgentOrchestrator)
└── api/                    # FastAPI app: main, routes, schemas, deps
tests/                       # 97 tests, one file per package above, + fixtures/
```
